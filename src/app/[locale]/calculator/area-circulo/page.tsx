...
export default function AreaCirculoCalculator() {
  ...
  const calculateArea = () => {
    console.log('[DEBUG] calculateArea called with radius:', radius)
    const r = parseFloat(radius)

    if (!isNaN(r) && r > 0) {
      const calculatedArea = Math.PI * Math.pow(r, 2)
      console.log('[DEBUG] Calculated area:', calculatedArea)
      setArea(calculatedArea)
      console.log('[DEBUG] State area set to:', calculatedArea)
    } else {
      console.log('[DEBUG] Invalid input, resetting area')
      ...
      <Card className="w-full max-w-lg">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex justify-center items-center">
            {/* Placeholder for circle image */}
            <svg width="150" height="150" viewBox="0 0 150 150" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="75" cy="75" r="60" stroke="black" strokeWidth="2"/>
              <line x1="75" y1="75" x2="135" y2="75" stroke="black" strokeWidth="1" />
              <text x="100" y="70" fontSize="12" fill="black">r</text>
            </svg>
          </div>
