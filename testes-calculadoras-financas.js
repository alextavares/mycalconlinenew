...
class TesteCalculadorasFinancas {
  constructor() {
    this.browser = null;
    this.context = null;
    this.page = null;
  ...
  async iniciar() {
    console.log('🚀 Iniciando testes das calculadoras financeiras...');
  ...
  async testarJurosSimples() {
    ...
    await this.page.goto('http://localhost:3000/pt-BR/calculator/juros-simples');
  ...
  async testarJurosCompostos() {
    ...
    await this.page.goto('http://localhost:3000/pt-BR/calculator/juros-compostos');
  ...
  async testarConversorMoeda() {
    console.log('\n💱 Testando Conversor de Moeda...');
    await this.page.goto('http://localhost:3000/pt-BR/calculator/currency-converter');
    
    try {
      await this.page.waitForTimeout(3000); // Aguardar carregamento das taxas
      
      // Teste 1: Conversão USD para EUR
      await this.page.fill('input[type="number"]', '100');
      
      // Selecionar USD como moeda de origem
      await this.page.click('[role="combobox"] >> nth=0');
      await this.page.click('text=USD');
      
      // Selecionar EUR como moeda de destino
      await this.page.click('[role="combobox"] >> nth=1');
      await this.page.click('text=EUR');
      
      await this.page.waitForTimeout(2000);
      
      const resultadoTexto = await this.page.textContent('text=/100 USD =/');
      
      if (resultadoTexto && resultadoTexto.includes('EUR')) {
        console.log('✅ Teste 1 - Conversor de Moeda: PASSOU');
        this.resultados.passou++;
      } else {
        console.log('❌ Teste 1 - Conversor de Moeda: FALHOU');
        this.resultados.erros.push({
          calculadora: 'Conversor de Moeda',
          teste: 'Conversão USD para EUR',
          esperado: 'Resultado em EUR',
          obtido: resultadoTexto || 'Nenhum resultado'
        });
        this.resultados.falhou++;
      }
      ...
      console.log('❌ Erro no teste de Conversor de Moeda:', error.message);
      this.resultados.erros.push({
        calculadora: 'Conversor de Moeda',
        erro: error.message
      });
  ...
  async testarPorcentagem() {
    ...
    await this.page.goto('http://localhost:3000/pt-BR/calculator/porcentagem');
  ...
  async testarGastoGasolina() {
    ...
    await this.page.goto('http://localhost:3000/pt-BR/calculator/gasto-gasolina');
  ...
  async executarTodosTestes() {
    try {
      ...
      await this.testarConversorMoeda();
...
if (require.main === module) {
  const testes = new TesteCalculadorasFinancas();
  testes.executarTodosTestes();
}

module.exports = TesteCalculadorasFinancas;